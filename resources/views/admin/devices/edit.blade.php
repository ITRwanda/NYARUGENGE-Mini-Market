    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-gray-100 flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0
                ...
                <i data-feather="cpu" class="w-5 h-5
        ...
        <form method="POST" action="{{ route('admin.devices.update', $device) }}" class="p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">
                        Assigned Stall
                        <span class="text-gray-400 font-normal text-xs">(determines which vendor sees this data)</span>
                    </label>
                    <select name="stall_id"
                        ...
                        @foreach($stalls as $stall)
                            <option value="{{ $stall->id }}"
                                    {{ old('stall_id', $device->stall_id) == $stall->id ? 'selected' : '' }}>
                                {{ $stall->stall_number }}
            ...
            <div class="pt-4 border-t border-dashed border-gray-200">
                <p class="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
                    <i data-feather="settings" class="w-4 h-4 text-gray-400"></i>Hardware Specification
                </p>
            ...
            <p class="text-xs text-gray-400 flex items-center gap-1.5">
                ...
                Last seen: {{ $device->last_seen_at->format('M d, Y H:i:s') }}
