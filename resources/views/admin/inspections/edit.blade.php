...
@section('subtitle', 'Update checklist and status')
@section('breadcrumb')
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-gray-100 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-gray-400">
                    {{ $market->name }} &bull;
                    Stall {{ $inspection->stall?->stall_number }} &bull;
                    {{ $inspection->inspection_date->format('M d, Y') }}
                </p>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <select name="stall_id" required
                        ...
                        @foreach($stalls as $stall)
                            <option value="{{ $stall->id }}"
                                    {{ old('stall_id', $inspection->stall_id) == $stall->id ? 'selected' : '' }}>
                                {{ $stall->stall_number }}
                ...
                <div>
                    ...
                    <textarea name="general_notes" rows="2"
                              class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none
                                     focus:ring-2 focus:ring-green-500/30 focus:border-green-500 resize-none">{{ old('general_notes', $inspection->general_notes) }}</textarea>
                </div>
            </div>
                    ...
                    <div class="checklist-row flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                        <div class="flex-1">
                            <input type="text" name="items[{{ $i }}][item]"
                                   value="{{ old("items.$i.item", $item->item) }}" required
        ...
        <form id="delete-form" method="POST" action="{{ route('admin.inspections.destroy', $inspection) }}" class="hidden">
            @csrf @method('DELETE')
        </form>
    </div>
</div>
...
function addItem() {
    ...
    idx++;
    document.getElementById('itemCount').textContent = document.querySelectorAll('.checklist-row').length;
}
