...
<head>
    ...
    <title>@yield('title', 'Dashboard') — Nyarugenge Market</title>
...
<div class="flex h-full min-h-screen">
    ...
    <aside id="sidebar"
           ...
           style="background: linear-gradient(180deg,#0a1a0a 0%,#102010 60%,#081508 100%);">
        ...
        <div class="flex items-center gap-3 px-5 py-5 border-b border-white/10">
            ...
            <div class="min-w-0">
                <p class="text-white font-bold text-sm leading-tight truncate">Nyarugenge</p>
                <p class="text-green-400 text-xs font-medium">Market Monitor</p>
            </div>
        </div>
        ...
        <nav class="flex-1 px-3 py-5 space-y-0.5 overflow-y-auto scrollbar-thin">
            ...
            {{-- ── Admin + Market Admin ── --}}
            @if($user->isAdmin() || $user->isMarketAdmin())
                <x-nav-section label="Management" />
                <x-nav-link route="admin.markets"   icon="map-pin"   label="Markets" />
            ...
            {{-- ── Admin + Market Admin + Inspector ── --}}
            @if($user->isAdmin() || $user->isMarketAdmin() || $user->isInspector())
                ...
                <x-nav-link route="admin.inspections" icon="clipboard" label="Inspections" />
            @endif

            {{-- ── Vendor portal ── --}}
            @if($user->isVendor())
                <x-nav-section label="My Space" />
                <x-nav-link route="admin.my-stall"    icon="box"      label="My Stall" />
    ...
    <div class="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        <header class="bg-white border-b border-gray-100 px-5 py-3.5 flex items-center justify-between sticky top-0 z-20 shadow-sm">
            <div class="flex items-center gap-3">
                <button onclick="toggleSidebar()"
                        class="text-gray-400 hover:text-gray-600 lg:hidden p-1">
                    <i data-feather="menu" class="w-5 h-5"></i>
                </button>
                <div>
                    <h1 class="text-base font-bold text-gray-900 leading-tight">@yield('title', 'Dashboard')</h1>
                    <p class="text-xs text-gray-400 hidden sm:block">@yield('subtitle', 'Nyarugenge Mini Market Monitoring System')</p>
                </div>
            </div>

            <div class="flex items-center gap-3">
                <div class="hidden md:flex items-center gap-1.5 text-sm text-gray-400 font-mono">
                    <i data-feather="clock" class="w-3.5 h-3.5"></i>
                    <span id="clock" class="text-xs"></span>
                </div>

                @if($user->isAdmin() || $user->isMarketAdmin() || $user->isInspector())
                <a href="{{ route('admin.alerts') }}" class="relative text-gray-400 hover:text-red-500 transition-colors p-1">
        ...
        <footer class="px-6 py-3 border-t border-gray-100 text-xs text-gray-400 text-center">
            © {{ date('Y') }} Nyarugenge Mini Market — Hygiene Monitoring System &bull; Rwanda
        </footer>
    </div>
</div>
